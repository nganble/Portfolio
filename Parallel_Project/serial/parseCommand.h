...
// -----------------------------------------------------------------
// Parse a command from a string "line" amd return a integer "value"
// Example:
//      line    = "-Nx=10"  (input)
//      command = "-Nx="    (input)
//      value   = 10        (output)
// Return value: 1 if command was not found in line
//               0 if not found
// -----------------------------------------------------------------
...
int parseCommand(const string &line, const string &command, double &value)
{
	...
	if (line.substr(0,len)==command)
	{
		...
		//printf("line.substr()==%s\n",line.substr(0,len).c_str());
		printf("parseCommand: SETTING %s%4.2f\n", command.c_str(), value);
		return 1;
	}
	return 0;
}
